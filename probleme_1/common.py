...
def echanger_points(
    solution: np.array, nbr_points: int, distance_actuelle: float
) -> tuple:
    """
...
def echanger_segment_consecutif(
    solution: np.array, nbr_points: int, distance_actuelle: float
) -> tuple:
    ...
    max_start = (
        n - nbr_points - 1 - 1
    )  # -1 pour end < n (car solution[end] doit exister)
    if max_start < 1:
        return solution.copy(), distance_actuelle, False
...
def deux_opt(
    solution: np.ndarray, recuit: bool = False, distance_actuelle: float = None
):
    """
